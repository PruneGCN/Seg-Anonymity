...
class SegLLM_KVCache_Manager:
    def __init__(
        ...
        print(f"Building SegLLM_KVCache_Manager: {initial_size}, {local_size}")
        self.initial_size = initial_size
        self.local_size = local_size
        # self.cache_size = initial_size + local_size
        ...
        if special_tokens_id is not None:
            ...
            if 'llama' in  model_type.lower():
                ...
                print("Here for pythia")
                self.special_tokens_id = [15, 13, 32, 2, 28, 27, 209, 186, 187,    964, 1157, 3736, 2195, 3706, 1163, 2490,  50276,    586, 4928, 50275 ]       # pythia 14b
    ...
    def __call__(self, past_key_values, CHECK_LOCAL_SIZE=True):
        if CHECK_LOCAL_SIZE:
            assert self.local_size==self.cache_size-self.initial_size, f"For streamingLLM, cache_size==local_size+initial_size"
            ...
            [
                torch.cat(
                    [
                        self.k_slice(k, 0, self.initial_size),
                        self.k_slice(
                            k, seq_len - self.local_size + num_coming, seq_len
                        ),
                    ],
                    dim=self.k_seq_dim,
                ),
                torch.cat(
                    [
                        self.v_slice(v, 0, self.initial_size),
                        self.v_slice(
                            v, seq_len - self.local_size + num_coming, seq_len
                        ),
                    ],
                    dim=self.v_seq_dim,
                ),
    ...
    def compress_kv_cache_and_tokids(self, past_key_values,  SEP_ACCUMULATION=False, USE_MAX_SEP_CACHE=False ):
        ...
        if USE_MAX_SEP_CACHE: ## Fixed sep cache size, i.e., only keep max_seg_len seps' kv in the cache. 
        ...
        if self.initial_size > 0:                                
            past_key_values, self.past_tok_ids  = self.cat_kv_cache_and_tokids( [initial_kv, seg_kv, local_kv ] ,  [initial_tokids, seg_tokids, local_tokids  ] )
        else:
            past_key_values, self.past_tok_ids  = self.cat_kv_cache_and_tokids( [seg_kv, local_kv ] ,  [seg_tokids, local_tokids  ] )
