...
class ParallelSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        # self.block_mask = neox_args.trnMask.block_mask 
        ##########################################################
    ...
    def attention(
        self, query_layer, key_layer, value_layer, layer_past, attention_mask, block_mask=None 
    ):
        # ===================================
        ...
        # ###############################################my#################################################
        # import os
        # if isinstance(attention_mask, list): # Deprecated
        #     attention_mask = attention_mask[self.layer_number]  # Deprecated
        # torch.set_printoptions(profile="full")
        # print(f">>>>>>>>>>>>>>>>>>>attention_mask for layer {self.layer_number}:  RANK: {os.getenv('RANK')}:  {(~attention_mask)[:,:,100,:]}.<<<<<<<<<<<<<<<<<<<")
        # torch.set_printoptions(profile="default")
        # ##################################################################################################        

        if self.USE_FLEX :
            # query = query_layer.permute(1,2,0,3).contiguous()
            # key = key_layer.permute(1,2,0,3).contiguous()
            # value = value_layer.permute(1,2,0,3).contiguous()
            ...
            # ##################################################################
            # print(f"############# USE FLEX in my attention")
            # print(f"Debug:  #########################query.shape: {query.shape}") ##torch.Size([32, 12, 2048, 64])
            # print(f"Debug:  #########################key.shape: {key.shape}")  ## torch.Size([32, 12, 2048, 64])
            # print(f"Debug:  #########################value.shape: {value.shape}") ##value.shape: torch.Size([32, 12, 2048, 64])
            

            # B, H, Sq, D = query.shape
            # Sk = key.shape[2]
            # idx0 = torch.zeros(1, dtype=torch.int, device=query.device)
            # # print(f"######################{[B, H, Sq, D, Sk]}")
            ...
            # block_mask = create_block_mask_cached(mask_mod, B, 1, Sq, Sk, device=query.device)
            # ##################################################################
            assert block_mask is not None
            
            context_layer =  flex_attention(query, key, value, score_mod=None, block_mask=block_mask)        
            return context_layer
        else:
            # print(f"############# USE common global in my attention")
            # #######################Debug##############################
            # query = query_layer.permute(1,2,0,3)
            # key = key_layer.permute(1,2,0,3)
            # value = value_layer.permute(1,2,0,3)
            # tmp_res = query @ key.transpose(2, 3)
            # #####################################################
            ...
            if self.USE_SA_SOFTMAX:                    
                ...
                # print(f"Debug#################attention_probs.shape:{attention_probs.shape}##########################")
                # # attention_probs.shape:torch.Size([32, 12, 2048, 2048]) ## B x  head x seq x seq
                # print(f"Debug#################attention_probs:{attention_probs}##########################")
                # print(f"Debug#################attention_mask:{attention_mask}##########################")


                # ###################################v0####################################
                # min_attention_score=torch.min(torch.tril(attention_scores), -1, keepdim=True)[0]
                # max_attention_score=torch.max(torch.tril(attention_scores), -1, keepdim=True)[0] 
                ...
                # attention_probs_1 = torch.tril(attention_probs).half()  ##my: For debug
                # attention_probs_2 = attention_probs.half()  ##my: For debug
                # print(f"Debug: ############attention_probs_1: {attention_probs_1.equal(attention_probs_2)}############")  ## True for both   "scaled-upper-triang-masked-softmax-fusion"= true  and  "scaled_masked_softmax_fusion": true, ####my !!!!
                # print(f"Debug:  attention_probs_2:{attention_probs_2}")
                # #########################################################################

                # ###################################v1####################################
                attention_scores_triled = torch.tril(attention_scores)
                # print(f"Debug before: attention_scores_triled.grad_fn{attention_scores_triled.grad_fn}")

                min_attention_score=torch.min(attention_scores_triled, -1, keepdim=True)[0]
                max_attention_score=torch.max(attention_scores_triled, -1, keepdim=True)[0] 
                # print(f"Debug before: min_attention_score.grad_fn{min_attention_score.grad_fn}")
                # print(f"Debug before: max_attention_score.grad_fn{max_attention_score.grad_fn}")

                min_attention_score=torch.minimum(min_attention_score, torch.zeros(1, device= attention_scores.device))
                max_attention_score=torch.maximum(max_attention_score, torch.zeros(1, device= attention_scores.device))
                # print(f"Debug after: min_attention_score.grad_fn{min_attention_score.grad_fn}")
                # print(f"Debug after: max_attention_score.grad_fn{max_attention_score.grad_fn}")

                attention_probs=attention_probs * ( (attention_scores - min_attention_score) / (max_attention_score - min_attention_score + 1e-10) )
                attention_probs = attention_probs.half()
                # print(f"Debug: attention_probs.grad_fn{attention_probs.grad_fn}")

                # attention_probs_1 = torch.tril(attention_probs).half()  ##my: For debug
                # attention_probs_2 = attention_probs.half()  ##my: For debug
                # attention_probs = attention_probs_2 ##my: For debug
                # print(f"Debug: ############attention_probs_1: {attention_probs_1.equal(attention_probs_2)}############")  ## True for both   "scaled-upper-triang-masked-softmax-fusion"= true  and  "scaled_masked_softmax_fusion": true, ####my !!!!
                # print(f"Debug:  attention_probs_2:{attention_probs_2}")
                # #########################################################################
    ...
    def flash_attention_bk(self, query_layer, key_layer, value_layer):
        # [b, np, sq, sk]
        output_size = (
            query_layer.size(1),
            query_layer.size(2),
    ...
    def flash_attention(self, query_layer, key_layer, value_layer, attention_mask=None):
        ...
        if self.use_flash_attention and not self.use_triton:
            # print("######################self.use_flash_attention and not self.use_triton##########################") ##my
            ...
            if not self.training:
                # print(f"######################self.use_flash_attention and not self.use_triton,   self.training: {self.training}, is_causal: {is_causal}##########################") ##my
    ...
    def forward(self, hidden_states, attention_mask, layer_past=None, block_mask=None, inter_position_ids=None):
        ...
        if self.use_flash_attention:
            ...
            context_layer = self.attention(
                query_layer, key_layer, value_layer, layer_past, attention_mask, block_mask=block_mask
            )
        else:
...
class ParallelTransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        self.USE_FLEX = neox_args.USE_FLEX ##my. For Flex attention
        ######################################################################
    ...
    def forward(self, x, attention_mask, layer_past=None, inter_position_ids=None, block_mask=None):
        ...
        if self.gpt_j_residual:  ##my:  True for pythia
            ...
            attention_output, attention_bias = self.attention(
                x1, attention_mask_layer, layer_past=layer_past, inter_position_ids=inter_position_ids, block_mask=block_mask
            )
            if self.use_cache:
            ...
            # x = x + attn(ln1(x))
            attention_output, attention_bias = self.attention(
                self.input_layernorm(x), attention_mask_layer, layer_past=layer_past, inter_position_ids=inter_position_ids, block_mask=block_mask
            )
            if self.use_cache:
                attention_output, presents = attention_output
                self.layer_past = presents
...
class ParallelTransformerLayerPipe(ParallelTransformerLayer):
    ...
    def forward(self, args):
        ...
        #################################my###############################
        if self.USE_BiPE:
            # print(f"Debug len(args): {len(args)}: args: {args}")
            assert (
                len(args) == 3
            ), "If USE_BiPE=True, ParallelTransformerLayerPipe expects 3 arguments - hidden_states, inter_position_ids, and attention_mask"
            ...
            # assert (
            #     len(args) == 3
            # ), f"len(args) = {len(args)}: If USE_FLEX=True, ParallelTransformerLayerPipe expects 3 arguments - hidden_states, block_mask, and attention_mask"
            
            if len(args) == 3:
                # print(f"###############Debug len(args): {len(args)}  ")
                hidden_states, block_mask, attention_mask = args
            else:
                # print(f"###############Debug len(args): {len(args)}  ")
                hidden_states, block_mask, _, attention_mask = args
            ...
            assert (
                len(args) == 2
            ), "If USE_BiPE=False, ParallelTransformerLayerPipe expects 2 arguments - hidden_states and attention_mask"
            hidden_states, attention_mask = args
            inter_position_ids = None
        ##################################################################
        ...
        output, moe_loss = super().forward(hidden_states, attention_mask, inter_position_ids=inter_position_ids, block_mask=block_mask)
        # auxiliary output
        ...
        if self.USE_BiPE:
            ...
            return output, block_mask,  attention_mask
