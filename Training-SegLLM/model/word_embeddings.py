...
class EmbeddingPipe(Embedding):
    ...
    def forward(self, args):
        ...
        if self.USE_BiPE :
            assert (
                len(args) == 4
            ), f"When self.USE_BiPE=True, expected 4 arguments (input_ids, intra_position_ids, inter_position_ids, attention_mask), but got {len(args)}."
        
        elif self.USE_FLEX:
            assert (
                len(args) == 4
            ), f"When self.USE_FLEX=True, expected 4 arguments (input_ids, position_ids, block_mask, attention_mask), but got {len(args)}."        
        
        else:
            assert (
                len(args) == 3
            ), f"When self.USE_BiPE=False, expected 3 arguments (input_ids, position_ids, attention_mask), but got {len(args)}."

        if self.USE_FLEX:
            input_ids = args[0]
            position_ids = args[1]
            block_mask = args[2]
            attention_mask = args[-1]  ## overide inter_position_ids when USE_BiPE=False

        else:
            input_ids = args[0]
            intra_position_ids = args[1]
            inter_position_ids = args[2]  ##when USE_BiPE=False, args[2]==args[-1] is attention_mask
            attention_mask = args[-1]  ## overide inter_position_ids when USE_BiPE=False since 

            position_ids = intra_position_ids

        # print(f"Debug: args len: {len(args)} intra_position_ids:{intra_position_ids}, inter_position_ids:{inter_position_ids}")

        embeddings = super().forward(input_ids, position_ids) ##For BiPE

        if self.USE_BiPE:
            # print(f"Debug in forward of EmbeddingPipe: self.USE_BiPE: {self.USE_BiPE}")
            return embeddings, inter_position_ids, attention_mask
        
        elif self.USE_FLEX:
            # print(f"Debug: ######################self.USE_FLEX:####################################")
            return embeddings,  block_mask,  attention_mask
        #####################################################


        # embeddings = super().forward(input_ids, position_ids) ##my. Be default
