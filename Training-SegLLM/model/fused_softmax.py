...
class FusedScaleMaskSoftmax(nn.Module):
    ...
    def forward_fused_softmax(self, input, mask):
        ...
        if self.upper_triang_mask_fusion:  
            # print(f"Debug: #############self.upper_triang_mask_fusion: True#####################") ##my
            ...
            # ###############################################my#################################################
            # print(f"Debug: #############self.upper_triang_mask_fusion: {self.upper_triang_mask_fusion}. slef.general_mask_fusion: {self.general_mask_fusion} #####################") ##my
            # self.upper_triang_mask_fusion: False. slef.general_mask_fusion: True
            
            # # print(f"######################in scaled_masked_softmax_fusion branch:  self.general_mask_fusion:{self.general_mask_fusion}##################################")
            # import os
            # attention_mask = mask
            # print(f"##############in scaled_masked_softmax_fusion branch, {attention_mask.shape}#############################") #[2, 1, 2048, 2048]
            # torch.set_printoptions(profile="full")
            # print(f">>>>>>>>>>>>>>>>>>>attention_mask RANK: {os.getenv('RANK')}:  {(~attention_mask)[:,:,100,:]}.<<<<<<<<<<<<<<<<<<<")
