...
class RotaryEmbedding(torch.nn.Module):
    ...
    # def forward(self, x, seq_dim=0, seq_len=None, USE_BiPE=False, inter_position_ids=None):
    def forward(self, x, seq_dim=0, seq_len=None):
        # if USE_BiPE:
        #     assert not (inter_position_ids is None), f"inter_position_ids:{inter_position_ids} must NOT be none"
        #     sin, cos = self.BiPE(inter_position_ids)
        #     return sin.to(x.device), cos.to(x.device)        
...
#     q, k, cos, sin, offset: int = 0 ):  # jitting fails with bf16
#     cos, sin = (
#         cos[offset : q.shape[0] + offset, ...],
#         sin[offset : q.shape[0] + offset, ...],
#     )
...
def apply_rotary_pos_emb(q, k, cos, sin, offset: int = 0, \
    ...
    # print(f"Debug: cos.shape:{cos.shape}, sin.shape:{sin.shape} ")  ## cos.shape:[2048, 1, 1, 16], sin.shape:[2048, 1, 1, 16] #[ seq_len , batch, heads, selected_dim]; selected_dim = head_dim * neox_args.rotary_pct
    # print(f"Debug: q.shape:{q.shape}, k.shape:{k.shape} ")  ## q.shape:[2048, 2, 12, 16], k.shape:[2048, 2, 12, 16]; #[ seq_len , batch, heads, selected_dim] ; selected_dim = head_dim * neox_args.rotary_pct

    # print(f"Debug: inter_position_ids.shape:{inter_position_ids.shape}, inter_position_ids: {inter_position_ids}")  ## inter_position_ids.shape:[2, 2048]; [bz , seq]
    ...
    if USE_BiPE and (inter_position_ids is not None):
        ...
        # print(f"Debug: After1:  cos.shape:{cos.shape}, sin.shape:{sin.shape} ")  # cos.shape:[2048, 16], sin.shape:[2048, 16] 
        # print(f"Debug: inter_position_ids.shape:{inter_position_ids.shape}")  # inter_position_ids.shape:[2, 2048]
        ...
        # print(f"Debug: After2:  cos.shape:{cos.shape}, sin.shape:{sin.shape} ")  
        # # Debug: After2:  cos.shape:[2048, 2, 1, 16], sin.shape:[2048, 2, 1, 16]
