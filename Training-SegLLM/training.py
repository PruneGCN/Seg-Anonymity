...
def pretrain(neox_args):
    ...
    """
    ##########################my###########################    
    if not neox_args.original_flag:                
        neox_args.trnMask = TrainingMask(neox_args)
        if neox_args.streamingLLM:
            neox_args.trnMask.special_tokens_id = [-1]
            neox_args.special_tokens_id = [-1]
            assert neox_args.trnMask.att_sink_max_idx > 0
            assert neox_args.att_sink_max_idx > 0

    else:
        neox_args.trnMask = None    
    #######################################################
    ...
    ################################my################################
    print(f"##################lr_scheduler.num_iters: {lr_scheduler.num_iters}##########################")
...
def forward_step(
    ...
    # print("######################tokens########################")
    # print(tokens.shape)  # B x seq_len. Actully, it's the input_ids for HuggingFace transformer format. No padding tokens in it when pretraining
    # print(tokens)
    ...
    # print(attention_mask.shape) # 1 x 1 x seq_len x seq_len. By default, the 1st dim is not B
        ...
        if input_ids.shape[-1] > 1: # for prefilling
            ...
            if  trnMask.PRINT_KV_RATIO: # when evaluating, print results
                ...
                if trnMask.print_KV_count % trnMask.print_KV_intervals == 0:
                    print("#############################print_KV_count in forward_step###############################")
                    print(f"trnMask.print_KV_count:     {trnMask.print_KV_count}")
...
def train_step_pipe(neox_args, timers, model, data_iterator):
    ...
    # print(f"######################in train_step_pipe trnMask.print_KV_count: {neox_args.trnMask.print_KV_count}########################")
